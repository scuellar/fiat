...
Module FinT.
  ...
  Ltac build_fully_determined_type':= 
    ...
    (let a' := fresh in
     ...
     unfold Heading.Domain, Tuple.GetAttribute, Tuple.GetAttributeRaw
       in *; simpl in *; intros; decompose_source_predicate;
     subst_projections; subst_invertible_functions; reflexivity).
  Ltac ExtractSource' :=
  solve
  [ simpl; intros; eapply CorrectDecoderEmpty;
    [ build_fully_determined_type' | decide_data_invariant ] ].
  Ltac extract_view' :=
  match goal with
  | |- context [ CorrectDecoder _ _ _ _ empty_Format _ _ _ ] =>
        ExtractSource'
  | H:cache_inv_Property ?mnd _
    |- CorrectRefinedDecoder _ _ _ _ (_ ++ _) _ _ _ _ => ExtractView
  end.
  Ltac apply_rules' :=
    first
      [ extract_view'
  ...
  sequence_four_tactics ltac:(start_synthesizing_decoder)
   ltac:(normalize_format; apply_rules')
   ...
   ltac:(continue_on_fail_2) ltac:(continue_on_fail_1)
   ltac:(continue_on_fail).
  ...
  Let format :=
        format_word ◦ (fin2Word 8 ∘ stationID)
                    ++ format_word ◦ data.
  Let format' :=
  ...
  Let invariant' (msg:sensor_msg'):=
        stationID' msg < pow2 8.
  Let enc_dec' : EncoderDecoderPair format' invariant'.
  Proof.
    (* derive_encoder_decoder_pair. *)
    econstructor; [ synthesize_aligned_encoder | synthesize_aligned_decoder' ].
  Defined.
  Let encode' := encoder_impl enc_dec'.
  Let decode' := decoder_impl enc_dec'.
  ...
  Let invariant (msg:sensor_msg):=
        Fin2Restrict.f2n (stationID msg) < pow2 8.
...
Lemma append_bit_WS_comm
: forall {sz} (w : word sz) (b1 b2 : bool),
  append_bit b2 (WS b1 w) = WS b1 (append_bit b2 w).
Proof. induction w; auto. Qed.
...
Theorem inverse_split 
: forall (n1 n2 : nat) (s : word (n1 + n2)) (v1 : word n1) (v2 : word n2),
  split1 n1 n2 s = v1 /\ split2 n1 n2 s = v2 -> s = Word.combine v1 v2.
Proof.
  intros. destruct H. rewrite <- H. rewrite <- H0.
  rewrite Word.combine_split. reflexivity.
Qed.
...
Ltac inverter_step :=
  ...
  | H : wreverse _ = _            |- _ =>
    apply inverse_wreverse in H
  | H1 : split1 ?n1 ?n2 ?s = ?v1,
    H2 : split2 ?n1 ?n2 ?s = ?v2  |- _ =>
    pose proof (inverse_split n1 n2 s v1 v2 (conj H1 H2));
    clear H1; clear H2
  | Heq : ?s = _  |- ?s = _ =>
    exact Heq
  end.

Ltac new_fully_determined_type :=
  repeat match goal with
         | H : _ /\ _ |- _ => destruct H
         | |- _ => intros
         end;
  repeat inverter_step.

(* Ltac ExtractSource ::= *)
(*      solve [simpl; intros; *)
(*        eapply CorrectDecoderEmpty; *)
(*          [ try build_fully_determined_type; *)
(*            new_fully_determined_type *)
(*          | decide_data_invariant ] ]. *)
...
Let enc_dec_automatic24:
  EncoderDecoderPair format_int24 (constant True).
  Proof.
    econstructor; [ synthesize_aligned_encoder | synthesize_aligned_decoder' ].
  Qed.
